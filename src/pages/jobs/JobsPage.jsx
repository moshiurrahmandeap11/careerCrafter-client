...
const JobsPage = () => {
  ...
  useEffect(() => {
    filterJobs();
  }, [jobs, searchTerm, location, salaryFilter, dateFilter, jobTypeFilter, experienceFilter, industryFilter, activeTab, sortBy]);
  ...
  const filterJobs = () => {
    ...
    if (activeTab === 'saved') {
      ...
      filtered = filtered.filter(job => job.featured || job.applications > 10);
    }
    ...
    filtered.sort((a, b) => {
      switch (sortBy) {
        ...
        case 'salary-low':
          return (a.salaryMax + a.salaryMin) / 2 - (b.salaryMax + b.salaryMin) / 2;
        case 'applications':
  ...
  const jobStats = useMemo(() => {
    ...
    const featuredJobs = jobs.filter(job => job.featured || job.applications > 10).length;
    const remoteJobs = jobs.filter(job => job.workMode === 'remote').length;
    const highSalaryJobs = jobs.filter(job => (job.salaryMin + job.salaryMax) / 2 > 100000).length;
        ...
        >
          <div className="text-center mb-8">
            ...
            <motion.p 
              ...
              transition={{ delay: 0.2 }}
            >
              Explore {jobs.length} curated opportunities from top companies worldwide
            </motion.p>
          </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            ...
            >
              ...
              <div className="text-2xl font-bold text-gray-900">{jobStats.totalJobs}</div>
            ...
            >
              ...
              <div className="text-2xl font-bold text-gray-900">{jobStats.featuredJobs}</div>
            ...
            >
              ...
              <div className="text-2xl font-bold text-gray-900">{jobStats.remoteJobs}</div>
            ...
            >
              ...
              <div className="text-2xl font-bold text-gray-900">{jobStats.highSalaryJobs}</div>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {jobTypeFilter && (
                <span className="bg-orange-100 text-orange-800 px-3 py-1 rounded-full text-sm flex items-center">
                  Type: {jobTypeFilter}
                  <button onClick={() => setJobTypeFilter('')} className="ml-2 hover:text-orange-600">×</button>
                </span>
              )}
              {experienceFilter && (
                <span className="bg-red-100 text-red-800 px-3 py-1 rounded-full text-sm flex items-center">
                  Experience: {experienceFilter}
                  <button onClick={() => setExperienceFilter('')} className="ml-2 hover:text-red-600">×</button>
                </span>
              )}
              {industryFilter && (
                <span className="bg-indigo-100 text-indigo-800 px-3 py-1 rounded-full text-sm flex items-center">
                  Industry: {industryFilter}
                  <button onClick={() => setIndustryFilter('')} className="ml-2 hover:text-indigo-600">×</button>
                </span>
              )}
            </div>
            ...
            {filteredJobs.map((job, index) => (
              ...
              >
                ...
                <div className={`relative ${viewMode === 'list' ? 'w-32 flex-shrink-0' : ''}`}>
                  ...
                  >
                    <Bookmark 
                      className={`w-4 h-4 ${savedJobs.includes(job._id) ? 'fill-current' : ''}`} 
                    />
                  </button>
                </div>
                      ...
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex-1">
                          ...
                          <div className="flex items-center gap-2 text-sm text-gray-600 mb-3">
                            ...
                            <span className="font-medium">{job.company || job.userName}</span>
                            {job.verified && (
          ...
          >
            ...
            <h3 className="text-2xl font-bold text-gray-900 mb-3">
              {activeTab === 'saved' ? 'No Saved Jobs Yet' : 
               ...
               activeTab === 'featured' ? 'No Featured Jobs' :
               'No Jobs Found'}
            </h3>
